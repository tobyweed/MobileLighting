...
//            try socket.accept(onInterface: "", port: 0) // "" -> no restriction
            ...
            print(" -- PhotoReceiver: Error while listening to port for connections.")
        }
    ...
    func netServiceDidPublish(_ sender: NetService) {
        ...
        print(" -- PhotoReceiver: PhotoReceiver did publish on port \(thisService.port) in domain \(thisService.domain) under name \(thisService.name)")
    }
    ...
    func socketDidDisconnect(_ sock: GCDAsyncSocket, withError err: Error?) {
        ...
        print(" -- PhotoReceiver: PhotoReceiver disconnected...")
        self.socket = nil
    ...
    func readPacket() {
        ...
        if self.socket.isConnected {
            ...
            print(" -- PhotoReceiver: readPacket: cannot read packet; socket disconnected.")
        }
    ...
    func socket(_ sock: GCDAsyncSocket, didRead data: Data, withTag tag: Int) {
        ...
        case 1:
            ...
            guard data.count == 4 else {
                print(" -- PhotoReceiver: Error in receiving packet: header of unexpected length.")
                return
            }
...
class DataWriter {
    func write(data: Data, path: String) {
        ...
        do {
            ...
            print(" -- PhotoReceiver: Successfully saved photo data to file \(fileURL).")
        } catch {
            print(" -- PhotoReceiver: Could not write photo data to file \(fileURL)")
        }
    }
}
...
class LensPositionReceiver: DataReceiver {
    ...
    func handle(packet: PhotoDataPacket) {
        guard let lensPosition = packet.lensPosition else {
            print(" -- PhotoReceiver: LensPositionReceiver: lens position missing from packet")
            return
        }
        print(" -- PhotoReceiver: Received lens position.")
    ...
    func receiveLensPositionSync() -> Float {
        ...
        photoReceiver.dataReceivers.insertFirst(
                LensPositionReceiver { (pos: Float) in
                    print(" -- PhotoReceiver: Lens position:\t\(pos)")
                    done = true
                    lensPos = pos
            }
        )
...
class StatusUpdateReceiver: DataReceiver {
    ...
    func handle(packet: PhotoDataPacket) {
        print(" -- PhotoReceiver: Received status update.")
...
class CalibrationImageReceiver: DataWriter, DataReceiver {
    ...
    func handle(packet: PhotoDataPacket) {
        ...
        print(" -- PhotoReceiver: Received calibration image.")
        completionHandler()
    }
...
class DecodedImageReceiver: DataWriter, DataReceiver {
    ...
    func handle(packet: PhotoDataPacket) {
        write(data: packet.photoData, path: path)
        print(" -- PhotoReceiver: Received decoded image.")
...
class SceneMetadataReceiver: DataWriter, DataReceiver {
    ...
    func handle(packet: PhotoDataPacket) {
        write(data: packet.photoData, path: path)
        print(" -- PhotoReceiver: Received scene metadata.")
        completionHandler()
    }
...
class ExposureReceiver: DataReceiver {
    ...
    func handle(packet: PhotoDataPacket) {
        print(" -- PhotoReceiver: Received exposure.")
        guard let exposure = packet.exposure else {
            print(" -- PhotoReceiver: Could not receive exposure.")
            return
        }
...
class AmbientImageReceiver: DataWriter, DataReceiver {
    ...
    let path: String
    func handle(packet: PhotoDataPacket) {
        print(" -- PhotoReceiver: Received ambient photo.")
        write(data: packet.photoData, path: path)
        completionHandler()
    }
    init(_ completionHandler: @escaping BlankHandler, path: String) {
...
class AmbientVideoReceiver: DataWriter, DataReceiver {
    ...
    let path: String
    func handle(packet: PhotoDataPacket) {
        print(" -- PhotoReceiver: Received video.")
...
class IMUDataReceiver: DataWriter, DataReceiver {
    let completionHandler: BlankHandler
    let path: String
    func handle(packet: PhotoDataPacket) {
        print(" -- PhotoReceiver: Received IMU data.")
        write(data: packet.photoData, path: path)
        completionHandler()
    }
    init(_ completionHandler: @escaping BlankHandler, path: String) {
